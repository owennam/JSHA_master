...
### Phase 3.6: 모바일 레이아웃 최적화 ✅ (2026-01-16 완료)
- [x] **메인 컨테이너 반응형**: `flex-col lg:flex-row` (모바일 세로, PC 가로)
- [x] **태그 뱃지 줄바꿈**: `flex-wrap` 추가 (좁은 화면에서 자동 줄바꿈)
- [x] **제목/버튼 반응형**: `flex-col sm:flex-row` (모바일에서 버튼 아래 배치)
...
### 11. 모바일 레이아웃 깨짐 (Flex 방향)

| 증상 | 원인 | 해결 |
|------|------|------|
| 사이드바와 영상이 한 줄에 찌그러짐 | `flex` 컨테이너가 모바일에서도 가로(row) 유지 | `flex-col lg:flex-row` 반응형 적용 |
| 태그 뱃지가 잘림 | 공간 부족 시 줄바꿈 안됨 | `flex-wrap` 추가 |
| 제목/버튼이 겹침 | 좁은 화면에서 가로 배치 유지 | `flex-col sm:flex-row` 적용 |

**패턴**: Tailwind 반응형 접두사 활용
```css
/* 모바일 우선 (기본값) → 큰 화면에서 변경 */
flex-col lg:flex-row   /* 모바일: 세로, PC: 가로 */
flex-col sm:flex-row   /* 모바일: 세로, 태블릿+: 가로 */
flex-wrap              /* 공간 부족 시 자동 줄바꿈 */
```

---

## 🛡️ 보안 및 배포 회고 (2026-01-17)

### 12. React2Shell (CVE-2025-55182) 보안 경고 분석

| 항목 | 내용 | 결론 |
|------|------|------|
| **경고 내용** | Next.js Flight 프로토콜을 통한 RCE 취약점 (CVSS 10.0) | 심각함 |
| **프로젝트 상태** | Vite + React SPA (Next.js 미사용) | **영향 없음 ✅** |

**교훈**: 보안 경고가 뜨면 **내 프로젝트의 기술 스택(Framework, Version)**과 일치하는지 먼저 분석해야 함. 무작정 업데이트하기보다 영향도를 파악하는 것이 우선.

### 13. npm 의존성 취약점 해결

`npm audit` 실행 결과 8개 취약점 발견.

| 조치 | 결과 |
|------|------|
| `npm audit fix` | High 5건, Moderate 1건 해결 ✅ |
| **남은 이슈** | `esbuild` 관련 Moderate 2건 (Vite 7.x 업그레이드 필요) |
| **결정** | 개발 서버(dev server) 관련 이슈이므로 프로덕션 빌드에는 영향 없음. **Vite 7.x 안정화 후 업데이트**하기로 결정. |

### 14. Skill 기반 자동화 도입

반복되는 작업을 `.agent/skills` 폴더에 정의항 에이전트가 활용할 수 있게 함.

- **`security-audit`**: 보안 점검 자동화 (npm audit, 버전 체크, 규칙 검사)
- **`deploy`**: 빌드 및 Firebase/Vercel 배포 절차 표준화

**이점**: "보안 점검해줘", "배포해줘" 한 마디로 복잡한 절차를 정확하게 수행 가능.
